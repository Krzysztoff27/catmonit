import { Group, Paper, Text, Center, Flex } from '@mantine/core';
...
function AlertListElement({ isWarning, onRemove }: AlertListElementProps) {
    return (
        ...
        bg="var(--background-color-6)" >
            <Group>
                <Center h="100%">
                    <DeviceTitle name="Tux" address="10.10.10.10" mr="150px" />
                    {isWarning ? (<IconAlertTriangleFilled color="red"/>) : (<IconExclamationCircleFilled color="red"/>)}
                    <Text ml="sm" c="red.7">Storage below 5%. Free up space.</Text>
                </Center>
                <Flex justify="flex-end" flex="1">
                    {/* might want to add  some pop-up when on hover 
                    and change color of the icon */}
                    <IconX stroke="3.5px" color="var(--background-color-3)"onClick={onRemove} style={{ cursor: 'pointer' }} />
                </Flex>
            </Group>
        </Paper>
    );
